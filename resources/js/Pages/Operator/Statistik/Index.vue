<template>
  <OperatorLayout>
    <Head title="Kelola Statistik Desa - Panel Operator" />
...
const handleStore = () => {
  newForm.post('/operator/statistik', {
    onSuccess: () => {
      newForm.reset();
      initBulkForms();
...
const handleBulkUpdate = (category) => {
  bulkForms[category].put('/operator/statistik/update-semua', {
    preserveScroll: true,
    onSuccess: () => {
...
const executeDelete = () => {
  deleting.value = true;
  router.delete(`/operator/statistik/${selectedItemId.value}`, {
    preserveScroll: true,
    onFinish: () => {
